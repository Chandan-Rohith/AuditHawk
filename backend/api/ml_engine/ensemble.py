"""
Ensemble Orchestrator  (Phase 1 Final - Bulletproof)
───────────────────────────────────────────────────────
Restored robust z-score architecture with Salami Failsafe and
poisoned-data quarantine.
"""
...
def run_pipeline(
    ...
    df["total_risk_index"] = pd.concat([
        ...
        df["ae_score"], 
        df["graph_score"], 
        df_for_scoring["velocity"], 
        df_for_scoring["rarity"] 
    ], axis=1).max(axis=1)
    
    if trusted_vendors:
        trusted_set = {v.strip().lower() for v in trusted_vendors}
        trusted_mask = df["merchant"].str.strip().str.lower().isin(trusted_set)
        # If trusted, completely ignore LOF, AE, Graph, and Rarity. ONLY flag Velocity/Pattern violations.
        df.loc[trusted_mask, "total_risk_index"] = df.loc[trusted_mask, ["velocity", "pattern"]].max(axis=1)
    
    # ── 5. Robust Thresholding (The Dynamic Failsafe & Quarantine) ──────
    
    # 🚨 THE SALAMI TRAP: Identify bot-volume immediately
    ...
    # Calculate the company's "normal" baseline using ONLY the unpoisoned data
    ...
    # Now calculate the exact Z-Score against the true, healthy corporate baseline
    df["robust_z_score"] = 0.6745 * (df["total_risk_index"] - risk_median) / mad

    # Combine dynamic ML anomalies with the hardcoded Salami mask
